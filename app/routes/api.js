...
module.exports = function(router) {
    ...
    var client = nodemailer.createTransport({
        service: 'gmail',
        auth: {
            user: 'munssn.project@gmail.com', // Your email address
            pass: 'Pixel8r0' // Your password
        },
    ...
    router.post('/users', function(req, res) {
        ...
        user.temporarytoken = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '1h' }); // Create a token for activating account through e-mail
            ...
            user.save(function(err) {
                if (err) {
                    ...
                    var email = {
                        ...
                        text: 'Hello ' + user.name + ', thank you for registering at MUNSSN. Please click on the following link to complete your activation: http://localhost:3000/activate/' + user.temporarytoken,
                        html: 'Hello<strong> ' + user.name + '</strong>,<br><br>Thank you for registering at MUNSSN. Please click on the link below to complete your activation:<br><br><a href="http://localhost:3000/activate/' + user.temporarytoken + '">Activate</a>'
                    };
    ...
    router.post('/checkusername', function(req, res) {
        User.findOne({ username: req.body.username }).select('username').exec(function(err, user) {
            if (err) {
                ...
                var email = {
                    from: 'MUNSSN',
                    to: 'munssn.project@gmail.com',
                    subject: 'Error Logged',
                    text: 'The following error has been reported to MUNSSN: ' + err,
                    html: 'The following error has been reported to MUNSSN:<br><br>' + err
                };
                ...
                if (!user) {
                    ...
                    if (!req.body.password) {
                        ...
                        if (!validPassword) {
                            ...
                            var token = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '1h' }); // Logged in: Give user token
    ...
    router.put('/resend', function(req, res) {
        User.findOne({ username: req.body.username }).select('username name email temporarytoken').exec(function(err, user) {
            if (err) {
                ...
                user.temporarytoken = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '1h' }); // Give the user a new token to reset password
                ...
                user.save(function(err) {
                    if (err) {
                        ...
                        var email = {
                            ...
                            html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently requested a new account activation link. Please click on the link below to complete your activation:<br><br><a href="http://localhost:3000/activate/' + user.temporarytoken + '">Activate</a>'
                        };
        ...
        User.findOne({ username: req.body.username }).select('username active email resettoken name').exec(function(err, user) {
            if (err) {
                ...
                if (!user) {
                    ...
                    user.resettoken = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '1h' }); // Create a token for activating account through e-mail
                    ...
                    user.save(function(err) {
                        if (err) {
                            ...
                            var email = {
                                ...
                                html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://localhost:3000/reset/' + user.resettoken + '">Reset</a>'
                            };
                            // Function to send e-mail to the user
                            client.sendMail(email, function(err, info) {
                                if (err) {
                    ...
                    user.save(function(err) {
                        if (err) {
                            ...
                            var email = {
                                ...
                                text: 'Hello ' + user.name + ', This e-mail is to notify you that your password was recently reset at MUNSSN',
                                html: 'Hello<strong> ' + user.name + '</strong>,<br><br>This e-mail is to notify you that your password was recently reset at MUNSSN'
                            };
                            // Function to send e-mail to the user
                            client.sendMail(email, function(err, info) {
        ...
        User.findOne({ username: req.params.username }).select('username email').exec(function(err, user) {
            if (err) {
                ...
                if (!user) {
                    ...
                    var newToken = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '1h' }); // Give user a new token
